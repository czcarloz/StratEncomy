...
export default function PlannedInvestmentsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setEntries(
        await api.plannedInvestments.list({
  ...
  async function handleCreate() {
    if (!form.asset_label || !form.amount_planned || !form.month || !form.year) {
      setFormError("All fields except note are required.");
      return;
    }
    setFormError("");
    setSubmitting(true);
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {entries.length > 0 && (
          <span className="text-sm text-zinc-400">
            Total: <span className="font-semibold text-zinc-100">{formatCurrency(total)}</span>
          </span>
        )}
        ...
        <div className="overflow-hidden rounded-lg border border-zinc-800">
          <table className="w-full text-sm">
            <thead className="border-b border-zinc-800 bg-zinc-900">
              <tr>
                <th className="px-4 py-3 text-left font-medium text-zinc-400">Period</th>
                <th className="px-4 py-3 text-left font-medium text-zinc-400">Asset</th>
                <th className="px-4 py-3 text-left font-medium text-zinc-400">Note</th>
                <th className="px-4 py-3 text-right font-medium text-zinc-400">Planned amount</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-800">
              {entries.map((entry) => (
                <tr key={entry.id} className="bg-zinc-950 hover:bg-zinc-900 transition-colors">
                  <td className="px-4 py-3 text-zinc-400">
                    {MONTHS[entry.month - 1]} {entry.year}
                  </td>
                  <td className="px-4 py-3 font-medium text-zinc-100">{entry.asset_label}</td>
                  <td className="px-4 py-3 text-zinc-500 max-w-xs truncate">{entry.note ?? "—"}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="New Planned Investment">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              placeholder="DCA strategy, rebalancing…"
              value={form.note}
