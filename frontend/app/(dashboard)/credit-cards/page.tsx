...
function CreateCardModal({
  ...
  async function handleSubmit() {
    if (!name || !closingDay || !dueDay) {
      setError("Name, closing day and due day are required.");
      return;
    }
    setError("");
    setLoading(true);
    ...
    <Modal open={open} onClose={onClose} title="New Credit Card">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <Label>Name</Label>
          <Input placeholder="Nubank, XP Visa…" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input type="number" min="0" step="0.01" placeholder="0.00" value={limit} onChange={(e) => setLimit(e.target.value)} />
        </div>
...
function AddPurchaseModal({
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      await api.creditCards.addPurchase(cardId, {
        description,
        total_amount: amount,
    ...
    <Modal open={open} onClose={onClose} title="Add Purchase">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <Input placeholder="Amazon, Netflix…" value={description} onChange={(e) => setDescription(e.target.value)} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1.5">
...
function CardDetail({
  ...
  const loadInvoice = useCallback(async () => {
    ...
    try {
      const inv = await api.creditCards.invoice(card.id, month, year);
      setInvoice(inv);
    } finally {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="text-zinc-400 hover:text-zinc-100 transition-colors text-sm">
          ← Cards
        </button>
        ...
        <span className="ml-1 text-zinc-500 text-xs">
          ...
          {card.limit ? ` · Limit ${formatCurrency(card.limit)}` : ""}
        </span>
        <div className="ml-auto flex gap-2">
          <Button onClick={() => setPurchaseModal(true)}>
            <Plus size={15} /> Add purchase
          </Button>
          <Button variant="danger" size="sm" onClick={onDeleteCard}>
      ...
      <div className="flex items-center gap-3">
        ...
        />
        {invoice && (
          <div className="ml-auto text-lg font-semibold text-zinc-100">
            Invoice total: <span className="text-indigo-400">{formatCurrency(invoice.total)}</span>
          </div>
        )}
      </div>
      ...
      {loadingInvoice ? (
        ...
        <div className="rounded-lg border border-zinc-800 py-10 text-center text-sm text-zinc-500">
        ...
        <div className="overflow-hidden rounded-lg border border-zinc-800">
          <table className="w-full text-sm">
            <thead className="border-b border-zinc-800 bg-zinc-900">
              <tr>
                <th className="px-4 py-3 text-left font-medium text-zinc-400">Purchase date</th>
                <th className="px-4 py-3 text-left font-medium text-zinc-400">Description</th>
                <th className="px-4 py-3 text-left font-medium text-zinc-400">Installment</th>
            ...
            <tbody className="divide-y divide-zinc-800">
              {invoice.items.map((item) => (
                <tr key={`${item.purchase_id}-${item.installment_number}`} className="bg-zinc-950 hover:bg-zinc-900 transition-colors">
                  <td className="px-4 py-3 text-zinc-400">{formatDate(item.purchase_date)}</td>
                  <td className="px-4 py-3 text-zinc-100">{item.description}</td>
                  <td className="px-4 py-3">
                    <Badge className="bg-indigo-900/40 text-indigo-400">
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <CardDetail
          ...
          onDeleteCard={() => handleDeleteCard(selectedCard)}
        />
      </div>
    );
  }
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((card) => (
            <button
              ...
              className="flex items-center gap-4 rounded-lg border border-zinc-800 bg-zinc-900 p-4 text-left transition-colors hover:border-indigo-600 hover:bg-zinc-800"
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-indigo-600/20 text-indigo-400">
                <CardIcon size={18} />
              </div>
              <div className="flex-1 overflow-hidden">
                <p className="font-medium text-zinc-100 truncate">{card.name}</p>
                <p className="text-xs text-zinc-500">
                  Closes {card.closing_day} · Due {card.due_day}
